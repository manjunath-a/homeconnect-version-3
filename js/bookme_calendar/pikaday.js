...
}(this, function (moment)
{
    ...
    renderTable = function(opts, data)
    {
        ...
        '<a class="pika-reset" style="position: absolute; bottom: 5px">reset</a></div>' + renderSessions(opts) + '';
    },
    ...
    renderSessions = function(opts){
    	...
    	for(var i = 0; i < opts._sessions.length; i++){
    		...
    		html += '<div class="'+((avalilible == '0')? 'is-disabled' : '' )+'">'+
    			'<a class="pika-session-button" style="text-decoration: none; float:left; width:40%; margin-left: 2px; margin-bottom: 2px; -moz-box-sizing: content-box; -webkit-box-sizing: content-box; box-sizing: content-box;">'+secondsToHms(parseInt(data[0], 10))+'</a></div>';
    	}
    	html += '</div>';
    ...
    Pikaday.prototype = {
        ...
        config: function(options)
        {
            ...
            if(this._type != 'Session'){
            	$('pikaday_element').style.cssText = 'display:block; height: 260px; max-width: 225px';
            }else{
            	$('pikaday_element').style.cssText = 'display:block; height: 260px; max-width: 388px';
            }
            ...
            if(this.reloadp == true)
            	$('pikaday_element').style.cssText += 'float:left';
        ...
        refreshSessions: function(){
        	...
        	new Ajax.Request(this.baseUrl + 'bookme_calendar/index/session?isAjax=true&id='+this._id+'&date='+this._sessionDate.getFullYear()+'/'+this._sessionDate.getMonth()+'/'+this._sessionDate.getDate(), {
        		method: 'get',
        	    asynchronous: true,
                onSuccess: function(transport) {
        ...
        refreshMaxQty: function(){
        	if(this._o.field.value != ''){
	        	new Ajax.Request(this.baseUrl + 'bookme_calendar/index/maxqty?isAjax=true&id='+this._id+'&dates='+this._o.field.value+'&offset='+$('options_multidate_offset').value, {
	        		method: 'get',
	        	    asynchronous: true,
	                onSuccess: function(transport) {
	                	var response = transport.responseText.evalJSON();
